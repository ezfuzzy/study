...
// })
document
  .getElementById("editableTextboxBtn")
...
document
  .getElementById("editableTextbox")
...
function clock() {
  ...
  Target_clock.innerText = `${hours < 10 ? `0${hours}` : hours}:${
    minutes < 10 ? `0${minutes}` : minutes
  }:${seconds < 10 ? `0${seconds}` : seconds}`;
  Target_apm.innerText = `${apm}`;
}

function clear_temperatureBox() {
  // temparture 입력받아서 옷 추천해주기
