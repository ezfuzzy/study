...
function GalleryForm() {
  ...
  const handleDrop = (e) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const reg = /image/;
      if (!reg.test(tmp.type)) {
        //파일의 type 이 만일 정규표현식을 통과하지 못하면
  ...
  const handleChange = (e) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      reader.onload = (event) => {
        ...
        const result = imageFiles.map((item) => {
          if (item.name === tmp.name) {
            item.dataUrl = data; // dataUrl 이라는 방에 읽은 이미지 데이터(src)를 넣어준다
          }
          return item;
        });
        // state 변경하기
      ...
      <a
        ...
        <div
          ...
          {imageData.length > 0 ? (
            ...
            <strong>Drag and Drop files or Click Here</strong>
          )}
        </div>
      </a>
